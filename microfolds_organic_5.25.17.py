...
metal_cofactors={}
cofactors_group={}
cofactors_file=open('wt_taxonomy_important_metals_groups_align_all_ligands_and_nonredundant_and_chl_ca_10_md_3_rmsd_5_factor_1.4.txt','r')
...
Metals=['MG','CA','NA','BA','BE','SR','RA','FE','MN','CU','CD','OS','CO','NI','W','PT','MO','U','TA','V','AU','IR','Y','GD','RU','YB','SM','PD','AG','EU','RH','PR','RE','LU','TB','HF','HO','DY','ZR','CR','LA','CE','ER','AM','CM','TH','PU','SC','PA']
             ...
             for chain in model:
                 for residue in chain:
                     if residue.resname not in AA and residue.resname not in CF :
                      ...
                      if residue.resname not in cofactor:                       
                       # print(chain.id,residue.resname)
                        atom_in_res=[]
                        for atom in residue:
                            atom_in_res.append(atom.element)
                           
                        if any(x in Metals for x in atom_in_res)==True:
                            print ('metal')
                            continue
                        ...
                        if not os.path.exists('/home/hraanan/MicrofoldsPDBs/organic/'+microfold_dir):
                            os.makedirs('/home/hraanan/MicrofoldsPDBs/organic/'+microfold_dir)
                        ...
                        io.save('/home/hraanan/MicrofoldsPDBs/organic/'+microfold_dir+'/'+microfold_name+'.pdb', MicroSelect())
    except:
        e = sys.exc_info()[0]
        Error_out.write('xxx\n')
