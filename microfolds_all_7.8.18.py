...
def get_center(res_list,al): # get the geometric centar of list of atom coordinatins 
    coord = []
                     ...
                     for residue in chain:
                          if residue.resname not in AA and residue.resname not in CF:
                              ...
                              if residue.resname in cofactors_dict:
                                ...
                                for al in cofactors_dict[residue.resname]:
                                   ...
                                   # print(microfold_name)
                                    if not os.path.exists('f:/microfolds_8_2018/organic/'+microfold_dir):
                                        os.makedirs('f:/microfolds_8_2018/organic/'+microfold_dir)
                                    ...
                                    io.save('f:/microfolds_8_2018/organic/'+microfold_dir+'/'+microfold_name+'.pdb', MicroSelect())
                                    #atom_list_file.write(microfold_name+'\t'+';'.join(al)+'\n')
                                    nuc=1
                                continue
                              if residue.resname in chl:
                                    ...
                                    microfold_dir=residue.resname
                                    microfold_dir=microfold_dir.replace(' ','')                                                            
                                    if not os.path.exists('f:/microfolds_8_2018/chl/'+microfold_dir):
                                        os.makedirs('f:/microfolds_8_2018/chl/'+microfold_dir)
                                    Select = Bio.PDB.Select
                                    class MicroSelect(Select):
                                       def accept_residue(self, residue):
                                           if residue in all_neighbors and residue.resname!='HOH':
                                    ...
                                    io.set_structure(structure)
                                    #print('/home/hraanan/MicrofoldsPDBs/'+microfold_dir+'/'+microfold_name+'.pdb', MicroSelect())                        
                                    io.save('f:/microfolds_8_2018/chl/'+microfold_dir+'/'+microfold_name+'.pdb', MicroSelect())
                              ...
                              if residue.resname in heme:
                                    ...
                                    microfold_dir=residue.resname
                                    microfold_dir=microfold_dir.replace(' ','')                                                            
                                    if not os.path.exists('f:/microfolds_8_2018/heme/'+microfold_dir):
                                        os.makedirs('f:/microfolds_8_2018/heme/'+microfold_dir)
                                    Select = Bio.PDB.Select
                                    class MicroSelect(Select):
                                       def accept_residue(self, residue):
                                           if residue in all_neighbors and residue.resname!='HOH':
                                    ...
                                    io.set_structure(structure)
                                    #print('/home/hraanan/MicrofoldsPDBs/'+microfold_dir+'/'+microfold_name+'.pdb', MicroSelect())                        
                                    io.save('f:/microfolds_8_2018/heme/'+microfold_dir+'/'+microfold_name+'.pdb', MicroSelect())
                              ...
                              microfold_dir=microfold_dir.replace(' ','')
                              print(microfold_name)
                              if not os.path.exists('f:/microfolds_8_2018/metals/'+microfold_dir):
                                    os.makedirs('f:/microfolds_8_2018/metals/'+microfold_dir)
                              Select = Bio.PDB.Select
                              class MicroSelect(Select):
                                   def accept_residue(self, residue):
                                       if residue in all_neighbors and residue.resname!='HOH':
                              ...
                              io.set_structure(structure)
                                #print('/home/hraanan/MicrofoldsPDBs/'+microfold_dir+'/'+microfold_name+'.pdb', MicroSelect())                        
                              io.save('f:/microfolds_8_2018/metals/'+microfold_dir+'/'+microfold_name+'.pdb', MicroSelect())
